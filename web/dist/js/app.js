...
},{"./is-object":16,"./request":18,"./request-base":17,"emitter":1,"reduce":3}],16:[function(require,module,exports){
/**
...
var getJobs = function getJobs(query) {
	return request.get('/api/scrape').query(query).set('Accept', 'application/json').observe().map(function (res) {
		return res.body.data;
	}).map(function (jobs) {
		return jobs.map(function (job, index) {
			return Object.assign({}, job, { index: index, toggled: false });
		});
	...
	return stream.onNext(function (state) {
		return Object.assign({}, state, {
			jobs: state.jobs.map(function (job) {
				return job.index === parseInt(index, 10) ? Object.assign({}, job, { toggled: !job.toggled }) : job;
			})
		});
	});
};
...
var toggleSalary = function toggleSalary() {
	return stream.onNext(function (state) {
		return Object.assign({}, state, { salary: !state.salary });
	});
};
...
},{"../util/request":22,"rx":4}],20:[function(require,module,exports){
'use strict';
...
},{"./actions":19,"./ui":21,"./util/vdom":23,"rx":4}],21:[function(require,module,exports){
'use strict';
...
module.exports = function (_ref) {
	...
	return section('#ui', [header([h1('Job Scraper'), form('.search', [label('keywords:'), input({ props: { type: 'text', name: 'q' } }), select({ props: { name: 'site' } }, sites.map(function (site) {
		return option({
			attrs: {
				value: site.uri,
	...
	}))]), form('.filter', [label('filter:'), input({
		props: { type: 'text', name: 'filter' },
		on: { input: function input(ev) {
				return actions.filter(ev.target.value);
			} }
	}), input({
		props: { type: 'checkbox', name: 'salary' },
		on: { click: function click(ev) {
				return actions.toggleSalary();
			} }
	}), span('salary')]), button('.btn-search', { on: {
			click: function click(ev) {
				return actions.getJobs(formToData(document.querySelector('.search')));
			}
		} }, [i('.fa.fa-search'), 'Search'])]), section('#content', [ul('#jobs-listing', state.jobs.filter(function (job) {
		return state.filter ? job.title.match(state.filter) || job.info.match(state.filter) : true;
	}).filter(function (job) {
		return state.salary ? job.salary !== '' : true;
	}).map(function (job) {
		return li('.job-body', [button('.toggle-info', { on: { click: function click() {
					return actions.toggle(job.index);
				} } }, [i('.fa.fa-info'), 'More Info']), a('.job-link', { props: { target: '_blank', href: job.link } }, job.title), job.salary ? div('.job-salary', job.salary) : '', div('.job-info', {
...
},{"../util/vdom":23}],22:[function(require,module,exports){
'use strict';
...
},{"rx":4,"superagent":15}],23:[function(require,module,exports){
'use strict';
...
},{"snabbdom":13,"snabbdom/h":5,"snabbdom/modules/attributes":8,"snabbdom/modules/class":9,"snabbdom/modules/eventlisteners":10,"snabbdom/modules/props":11,"snabbdom/modules/style":12}]},{},[20]);
